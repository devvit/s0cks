...
class Hub {
  ...
  constructor(config) {
    ...
    _defineProperty(this, "_onServerConnection", conn => {
      ...
      updateConnStatus('new'); // create a relay for the current connection
  ...
  async run() {
    await this._config._ready; // terminate if it already started
  ...
  async terminate() {
    // udp relays
    this._udpRelays.reset(); // tcp relays


    this._tcpRelays.forEach(relay => relay.destroy());

    this._tcpRelays.clear(); // udp server


    this._udpServer && this._udpServer.close(); // server

    this._tcpServer.close(); // others
  ...
  async _createUdpServer() {
    return new Promise((resolve, reject) => {
      ...
      const server = _dgram.default.createSocket('udp4'); // destroy old relays every 5s
      ...
      server.on('error', reject); // monkey patch for Socket.send() to meet Socks5 protocol
  ...
  _createRelay(source) {
    ...
    return this._config.mux ? new MuxRelay(props) : new _relay.Relay(props);
  }
