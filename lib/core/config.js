...
class Config {
  ...
  constructor(json) {
    ...
    _defineProperty(this, "stores", []); // service
    ...
    this.local_pathname = pathname; // server

    let server; // TODO(remove in next version): make backwards compatibility to "json.servers"
    ...
    this._initLogger(json); // https_cert, https_key
    ...
    this.dns_expire = json.dns_expire !== undefined ? json.dns_expire * 1e3 : _utils.DNS_DEFAULT_EXPIRE; // dns
  ...
  async _initServer(server) {
    ...
    this.server_pathname = pathname; // preload tls_cert or tls_key
    ...
    this.udp_presets = server.presets; // acl
    ...
    if (server.acl_conf !== undefined && server.acl) {
      ...
      this.acl_rules = await ACL.loadRules(_path.default.resolve(process.cwd(), server.acl_conf));
    } // redirect
  ...
  _initLogger(json) {
    ...
    _utils.logger.configure({
      ...
      format: combine(timestamp(), splat(), // TODO: Enable coloring. Currently we have to prevent dumping color characters in log files.
      // colorize(),
      prettyPrint(), printf(info => `${info.timestamp} - ${info.level}: ${info.message}`)),
      transports: trans
    });
  }
  ...
  static testOnClient(json) {
    ...
    const {
      ...
      port: _port,
      searchParams
    } = new _url.URL(json.service); // service.protocol
    ...
    if (proto === 'tcp') {
      const forward = searchParams.get('forward'); // ?forward
    ...
    Config._testServer(server, true); // common
  ...
  static testOnServer(json) {
    // server
    Config._testServer(json, false); // redirect
  ...
  static _testServer(server, from_client) {
    ...
    const {
      ...
      hostname,
      port: _port
    } = new _url.URL(server.service); // service.protocol
