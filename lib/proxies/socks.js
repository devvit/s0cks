...
const REPLY_SUCCEEDED = 0x00; // const REPLY_FAILURE = 0x01;
...
const REPLY_COMMAND_NOT_SUPPORTED = 0x07; // const REPLY_ADDRESS_TYPE_NOT_SUPPORTED = 0x08;
...
function parseSocks4Request(buffer) {
  ...
  let DSTADDR = [];
  const isSocks4a = DSTIP[0] === NOOP && DSTIP[1] === NOOP && DSTIP[2] === NOOP && DSTIP[3] !== NOOP; // Socks4a
        ...
        if (request !== null) {
          ...
          switch (method) {
            case METHOD_NO_AUTH:
              ...
              stage = STAGE_SOCKS5_REQUEST_MESSAGE; // Socks5 Select Message

              socket.write(Buffer.from([SOCKS_VERSION_V5, METHOD_NO_AUTH]));
              break;

            case METHOD_USERNAME_PASSWORD:
              stage = STAGE_SOCKS5_INITIAL_NEGOTIATION_MESSAGE; // Socks5 Initial negotiation
            ...
            case REQUEST_COMMAND_UDP:
              {
                ...
                const port = (0, _utils.numberToBuffer)(bindPort); // Socks5 Reply Message
